...
const DataTable = (() => {
  ...
  function exportCSV() {
    ...
    const headers = ['Base', 'Clé registre', 'Pipeline Nautilus (projet)', 'Statut relation', 'Tags équipe',
      ...
      'Statut', 'Latitude', 'Longitude', 'Précision géo', 'Régime ICPE', 'ICPE nom', 'ICPE confiance', 'ICPE fiche',
      'Lien Google Maps (site)', 'Lien Google Maps (coordonnées)'];
    ...
    const rows = currentData.map(d => [
      ...
      d.geoPrecision || '',
      d.icpe ? d.icpe.lib_regime || d.icpe.regime || '' : '',
      d.icpe ? d.icpe.nom || '' : '',
      d.icpe ? d.icpe.confiance || '' : '',
      d.icpe ? d.icpe.url || '' : '',
